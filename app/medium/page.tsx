...
import { mediumFlights } from '../data/mediumFlights'

export default function MediumLoad() {
  const [aircraft, setAircraft] = useState<Aircraft[]>([])
  const [flights, setFlights] = useState<Flight[]>([])
  const [selectedDate, setSelectedDate] = useState('2025-08-18')

  useEffect(() => {
    // Use first 10 aircraft for medium load
    const mediumAircraft = backupAircraft.slice(0, 10)
    setAircraft(mediumAircraft)
    setFlights(mediumFlights)
  }, [])
      ...
      <div className="container mx-auto px-4 py-4 sm:py-8">
        <header className="text-center mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-4xl font-bold text-gray-900 mb-2">
            Emirates Airlines - Medium Load Dashboard
          </h1>
          <p className="text-sm sm:text-lg text-gray-600 mb-4 sm:mb-6">
            Aircraft-based flight grouping with connected pairs - Week view by default, showing 10 aircraft with 300 flights
          </p>
        ...
        <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
            <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 mb-2 sm:mb-0">
              Medium Load Flight Schedule - 10 Aircraft with 300 Flights
            </h2>
